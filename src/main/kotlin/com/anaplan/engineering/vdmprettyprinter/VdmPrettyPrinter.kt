...
class VdmPrettyPrinter(
    ...
    private fun renderSUnaryExp(node: SUnaryExp, token: RenderToken, context: IRenderContext, requiresSpace: Boolean = true) =
    ...
    private fun renderLeftRightSUnaryExp(node: SUnaryExp, left: RenderToken, right: RenderToken, context: IRenderContext) =
            builder(context). //
                    token(left). //
    ...
    override fun defaultPTraceDefinition(node: PTraceDefinition, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun caseATrapStm(node: ATrapStm, context: IRenderContext) =
            builder(context).token(trap).space().node(node.patternBind).space().token(withToken).space().node(node.with).space().token(inToken).space().node(node.body).render
    ...
    override fun caseAExistsExp(node: AExistsExp, context: IRenderContext) =
            builder(context).token(exists).space().nodeList(node.bindList).space().token(bullet).space().node(node.predicate).render
    ...
    override fun defaultPStmtAlternative(node: PStmtAlternative, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun defaultSNumericBinaryExp(node: SNumericBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseASetUnionBinaryExp(node: ASetUnionBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)
    ...
    override fun caseALambdaExp(node: ALambdaExp, context: IRenderContext) =
            builder(context).token(lambda).space().nodeList(node.bindList).space().token(bullet).space().node(node.expression).render
    ...
    override fun defaultSClassDefinition(node: SClassDefinition, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun defaultPAccessSpecifier(node: PAccessSpecifier, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseAExternalDefinition(node: AExternalDefinition, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun caseAImpliesBooleanBinaryExp(node: AImpliesBooleanBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseAOrBooleanBinaryExp(node: AOrBooleanBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseAIntNumericBasicType(node: AIntNumericBasicType, context: IRenderContext) = renderToken(int, context)
    ...
    override fun caseAIsOfBaseClassExp(node: AIsOfBaseClassExp, context: IRenderContext) =
            builder(context).token(isOfBaseClass).lparens().node(node.baseClass).token(comma).space().node(node.exp).rparens().render
    ...
    override fun caseAOperationType(node: AOperationType, context: IRenderContext) =
            builder(context).conditional(node.parameters.isEmpty(), {
                ...
                if (node.parameters.size == 1 && (node.parameters.first is AProductType || typeRequiresParentheses(node.parameters.first, node))) {
                    it.lparens().node(node.parameters.first).rparens()
                } else {
                    it.nodeList(nodeList = node.parameters, separators = listOf(space, multiplication, space), renderNode = { listNode, builder ->
                        if (typeRequiresParentheses(listNode as PType, AProductType::class) && node.parameters.size > 1) {
                            builder.lparens().node(listNode).rparens()
                        } else {
                            builder.node(listNode)
                        }
    ...
    override fun caseAAssignmentDefinition(node: AAssignmentDefinition, context: IRenderContext) =
            builder(context).token(dcl).space().node(node.name).token(colon).space().node(node.type).space().token(assign).space().node(node.expression).render

    override fun caseAUnknownType(node: AUnknownType, context: IRenderContext) = renderToken(anyType, context)

    override fun caseAStarStarBinaryExp(node: AStarStarBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun defaultPBind(node: PBind, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun caseALocalDefinition(node: ALocalDefinition, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseAPlusNumericBinaryExp(node: APlusNumericBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)
    ...
    override fun defaultPObjectDesignator(node: PObjectDesignator, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseARealNumericBasicType(node: ARealNumericBasicType, context: IRenderContext) = renderToken(real, context)

    override fun caseASkipStm(node: ASkipStm, context: IRenderContext) = renderToken(skip, context)

    override fun caseAInstanceTraceDefinition(node: AInstanceTraceDefinition, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseAForAllExp(node: AForAllExp, context: IRenderContext) =
            builder(context).token(forall).space().nodeList(node.bindList).space().token(bullet).space().node(node.predicate).render
    ...
    override fun caseAMapUnionBinaryExp(node: AMapUnionBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)
    ...
    override fun caseAMapletPatternMaplet(node: AMapletPatternMaplet, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseASubtractNumericBinaryExp(node: ASubtractNumericBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseASetRangeSetExp(node: ASetRangeSetExp, context: IRenderContext) =
            builder(context).token(lbrace).node(node.first).token(comma).space().token(range).token(comma).space().node(node.last).token(rbrace).render
    ...
    override fun caseClassDefinitionSettings(node: ClassDefinitionSettings, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseAApplyExpressionTraceCoreDefinition(node: AApplyExpressionTraceCoreDefinition, context: IRenderContext) =
    ...
    override fun caseANamePatternPair(node: ANamePatternPair, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseILexStringToken(node: ILexStringToken, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun defaultPTraceCoreDefinition(node: PTraceCoreDefinition, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseAElementsUnaryExp(node: AElementsUnaryExp, context: IRenderContext) = renderSUnaryExp(node, elems, context)
    ...
    override fun caseAOperationExport(node: AOperationExport, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun caseAInSetBinaryExp(node: AInSetBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseANatNumericBasicType(node: ANatNumericBasicType, context: IRenderContext) = renderToken(nat, context)

    override fun caseAPostOpExp(node: APostOpExp, context: IRenderContext) = builder(context).node(node.postexpression).render

    override fun defaultSSetExp(node: SSetExp, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun caseASetDifferenceBinaryExp(node: ASetDifferenceBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseAUndefinedType(node: AUndefinedType, context: IRenderContext) = renderToken(undefined, context)

    override fun caseARangeResByBinaryExp(node: ARangeResByBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseAForAllStm(node: AForAllStm, context: IRenderContext) =
            builder(context).token(forToken).space().token(all).space().node(node.pattern).space().token(inSet).space().node(node.set).nl(). //
                    token(doToken).space().node(node.statement).render
    ...
    override fun caseAUntypedDefinition(node: AUntypedDefinition, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun caseAAndBooleanBinaryExp(node: AAndBooleanBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseAPreExp(node: APreExp, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun defaultSSimpleBlockStm(node: SSimpleBlockStm, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseARepeatTraceDefinition(node: ARepeatTraceDefinition, context: IRenderContext) =
            builder(context).node(node.core).conditional(node.to != 1L, {
                it.conditional(node.from == node.to, {
                    ...
                    it.token(lbrace).token(node.from.toString()).token(comma).space().token(node.to.toString()).token(rbrace)
                })
            }).render
    ...
    override fun caseALessNumericBinaryExp(node: ALessNumericBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)
    ...
    override fun caseAInheritedDefinition(node: AInheritedDefinition, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun caseADistUnionUnaryExp(node: ADistUnionUnaryExp, context: IRenderContext) = renderSUnaryExp(node, dunion, context)
    ...
    override fun caseAConcurrentExpressionTraceCoreDefinition(node: AConcurrentExpressionTraceCoreDefinition, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun caseAImportedDefinition(node: AImportedDefinition, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseAProtectedAccess(node: AProtectedAccess, context: IRenderContext) = renderToken(protectedToken, context)

    override fun caseILexIdentifierToken(node: ILexIdentifierToken, context: IRenderContext) = renderToken(node.name, context)

    override fun caseAIsOfClassExp(node: AIsOfClassExp, context: IRenderContext) =
            builder(context).token(isOfClass).lparens().node(node.className).token(comma).space().node(node.exp).rparens().render

    override fun caseARenamedDefinition(node: ARenamedDefinition, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun caseILexIntegerToken(node: ILexIntegerToken, context: IRenderContext) = renderToken(node.value.toString(), context)

    override fun caseANotEqualBinaryExp(node: ANotEqualBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)
    ...
    override fun caseAMuExp(node: AMuExp, context: IRenderContext) =
            builder(context).token(mu).lparens().node(node.record).token(comma).space().nodeList(node.modifiers).rparens().render

    override fun caseANonDeterministicSimpleBlockStm(node: ANonDeterministicSimpleBlockStm, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseASameBaseClassExp(node: ASameBaseClassExp, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseAExists1Exp(node: AExists1Exp, context: IRenderContext) =
            builder(context).token(exists1).space().node(node.bind).space().token(bullet).space().node(node.predicate).render
    ...
    override fun caseAStateDefinition(node: AStateDefinition, context: IRenderContext) =
            builder(context).token(node.name.name).space().token(of).incIndent().nl(). //
                    ...
                    conditional(node.initPattern != null, {
                        it.nl().token(initToken).space().node(node.initPattern).space().token(fnEquals).space().node(node.initExpression)
                    }). //
                    nl().token(end).render
    ...
    override fun caseAProperSubsetBinaryExp(node: AProperSubsetBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)
    ...
    override fun caseAIntLiteralExp(node: AIntLiteralExp, context: IRenderContext) = renderToken(node.value.value.toString(), context)
    ...
    override fun caseARealLiteralExp(node: ARealLiteralExp, context: IRenderContext) = renderToken(node.value.value.toString(), context)
    ...
    override fun caseAForIndexStm(node: AForIndexStm, context: IRenderContext) =
            builder(context).token(forToken).space().node(node.`var`).space().token(equals).space().node(node.from).space().token(toToken).space().node(node.to). //
    ...
    override fun caseAMultiBindListDefinition(node: AMultiBindListDefinition, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun defaultPImports(node: PImports, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun defaultPStateDesignator(node: PStateDesignator, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun caseADomainResToBinaryExp(node: ADomainResToBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseAGreaterEqualNumericBinaryExp(node: AGreaterEqualNumericBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseAStateInitExp(node: AStateInitExp, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun caseASeqConcatBinaryExp(node: ASeqConcatBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)
    ...
    override fun caseAEquivalentBooleanBinaryExp(node: AEquivalentBooleanBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)
    ...
    override fun caseASetIntersectBinaryExp(node: ASetIntersectBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)
    ...
    override fun caseAMapUnionPattern(node: AMapUnionPattern, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseAEqRelation(node: AEqRelation, context: IRenderContext) =
            builder(context).node(node.lhsPattern).space().token(equals).space().node(node.rhsPattern).space().token(fnEquals).space().node(node.relExp).render
    ...
    override fun caseILexBooleanToken(node: ILexBooleanToken, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseAPeriodicStm(node: APeriodicStm, context: IRenderContext) =
            builder(context).token(periodic).lparens().nodeList(node.args).rparens().lparens().node(node.opname).rparens().render
    ...
    override fun caseAIndicesUnaryExp(node: AIndicesUnaryExp, context: IRenderContext) = renderSUnaryExp(node, inds, context)

    override fun caseAAllImport(node: AAllImport, context: IRenderContext) = builder(context).render

    override fun caseASpecificationStm(node: ASpecificationStm, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseILexCharacterToken(node: ILexCharacterToken, context: IRenderContext) = renderToken(node.value.toString(), context)
    ...
    override fun defaultSNumericBasicType(node: SNumericBasicType, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun defaultPAlternativeStm(node: PAlternativeStm, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseADivNumericBinaryExp(node: ADivNumericBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)
    ...
    override fun defaultSOperationDefinition(node: SOperationDefinition, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun caseANatOneNumericBasicType(node: ANatOneNumericBasicType, context: IRenderContext) = renderToken(nat1, context)
    ...
    override fun caseADivideNumericBinaryExp(node: ADivideNumericBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseAClassInvariantStm(node: AClassInvariantStm, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun defaultSBooleanBinaryExp(node: SBooleanBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseANotYetSpecifiedExp(node: ANotYetSpecifiedExp, context: IRenderContext) = renderToken(isNotYetSpecified, context)
    ...
    private fun renderClass(node: SClassDefinitionBase, classNavMarker: NavigationMarker, context: IRenderContext): String {
        val sectionTypes = mapOf(
                ATypeDefinition::class.java to NavigationMarker.types,
                AValueDefinition::class.java to NavigationMarker.values,
                SFunctionDefinition::class.java to NavigationMarker.functions,
    ...
    override fun caseADomainResByBinaryExp(node: ADomainResByBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)
    ...
    override fun caseALessEqualNumericBinaryExp(node: ALessEqualNumericBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)
    ...
    override fun caseAInMapMapType(node: AInMapMapType, context: IRenderContext) =
            builder(context).token(inmap).space().node(node.from).space().token(inmapTo).space().childType(node.to, node).render
    ...
    override fun caseAExplicitOperationDefinition(node: AExplicitOperationDefinition, context: IRenderContext) =
            builder(context). //
                    ...
                    token(node.name.name).lparens().nodeList(node.parameterPatterns).rparens().space().token(fnEquals).incIndent().nl(). //
                    node(node.body).decIndent(). //
                    conditional(node.precondition != null, {
                        it.nl().token(pre).space().node(node.precondition)
                    }). //
    ...
    override fun defaultSFunctionDefinition(node: SFunctionDefinition, context: IRenderContext) = unhandled(node.javaClass, context)
    ...
    override fun caseABracketedExpressionTraceCoreDefinition(node: ABracketedExpressionTraceCoreDefinition, context: IRenderContext) =
            builder(context).lparens().incIndent().nl(). //
    ...
    override fun caseANotInSetBinaryExp(node: ANotInSetBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseAModNumericBinaryExp(node: AModNumericBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)
    ...
    override fun caseARemNumericBinaryExp(node: ARemNumericBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseBoolean(node: Boolean, context: IRenderContext) = renderToken(node.toString(), context)

    override fun caseATimesNumericBinaryExp(node: ATimesNumericBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseASameClassExp(node: ASameClassExp, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun defaultSInvariantType(node: SInvariantType, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun caseAIdentifierPattern(node: AIdentifierPattern, context: IRenderContext) = renderToken(node.name.name, context)
    ...
    override fun caseATixeStmtAlternative(node: ATixeStmtAlternative, context: IRenderContext) = unhandled(node.javaClass, context)

    override fun caseAOrdRelation(node: AOrdRelation, context: IRenderContext) =
            builder(context).node(node.lhsPattern).space().token(lessThan).space().node(node.rhsPattern).space().token(fnEquals).space().node(node.relExp).render
    ...
    override fun caseARangeResToBinaryExp(node: ARangeResToBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)

    override fun caseAGreaterNumericBinaryExp(node: AGreaterNumericBinaryExp, context: IRenderContext) = renderSBinaryExp(node, context)
